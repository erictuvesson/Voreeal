...
public:
	...
	bool ReadHeader(FArchive& Ar, FString& ErrorMessage)
	{
		Ar << version;
		Ar << colorFormat;
		Ar << zAxisOrientation;
		...
		if (version != VER_1_0_0_0 &&
			version != VER_1_1_0_0)
		{
			ErrorMessage = "Wrong Qubicle version!";
			return false;
		}

		if (numMatrices == 0)
		{
			ErrorMessage = "Data is invalid. (MatricesCount)";
			return false;
		}
...
bool FQubicleBinaryFileFormat::IsFile(FArchive& Ar, FString& ErrorMessage)
{
	QubicleModel model;
	return model.ReadHeader(Ar, ErrorMessage);
}

bool FQubicleBinaryFileFormat::ReadFile(FArchive& Ar, UBasicVolume* Volume, FString& ErrorMessage)
{
	QubicleModel model;
	if (!model.ReadHeader(Ar, ErrorMessage))
	{
		return false;
	}
